...
namespace AtomicCore.BlockChain.ExplorerAPI
{
    ...
    /// </summary>
    internal static class BlockCypherCacheProvider
    {
        #region Variable
        ...
        public static void Set(string key, object value, BlockCypherCacheMode mode = BlockCypherCacheMode.None, TimeSpan? expriedTime = null)
        {
            if (string.IsNullOrEmpty(key))
                throw new ArgumentNullException(nameof(key));
                ...
                lock (dynamicKey)
                {
                    if (null == ck_value)
                    {
                        using (ICacheEntry cacheEntry = s_businessCache_instance.CreateEntry(key))
                        {
                            ...
                            if (mode == BlockCypherCacheMode.AbsoluteExpired)
                            {
                                if (null == expriedTime)
                            ...
                            else if (mode == BlockCypherCacheMode.SlideExpired)
                            {
                                if (null == expriedTime)
                                    throw new ArgumentNullException(nameof(expriedTime));
