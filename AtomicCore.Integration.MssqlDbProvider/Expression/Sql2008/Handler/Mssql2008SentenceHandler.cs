...
namespace AtomicCore.Integration.MssqlDbProvider
{
    ...
    /// Mssql语句脚本解析
    /// </summary>
    internal class Mssql2008SentenceHandler : ExpressionVisitorBase
    {
        #region Variables
        ...
        private Mssql2008SentenceHandler(IDbMappingHandler dbMappingHandler)
            : base()
        {
            this._dbMappingHandler = dbMappingHandler;
            this._result = Mssql2008SentenceResult.Create();
        }
        ...
        /// 最终解析的结果
        /// </summary>
        public Mssql2008SentenceResult Result
        {
            ...
            switch (methodCallExp.Method.Name)
            {
                case "Select":
                    ...
                    if (select_exp != null)
                    {
                        ...
                        {
                            Mssql2008SelectCombinedResult selectFieldResult = Mssql2008SelectCombinedHandler.ExecuteResolver(select_func_lambdaExp, this._dbMappingHandler);
                            //接收需要被查询的字段
        ...
        /// <returns></returns>
        public static Mssql2008SentenceResult ExecuteResolver(Expression exp, IDbMappingHandler dbMappingHandler)
        {
            //执行表达式解析 要被查询的字段
